...
function Rp(e, t) {
  ...
  if (a) {
    ...
    if (e.dependentArgumentPath) {
      ...
      t.addErrorMessage(
        (w) =>
          `Argument \`${w.green(o)}\` is required because argument \`${w.green(
            f
          )}\` was provided.`
      );
    }
  ...
  switch (n) {
    ...
    case "parameterTuple":
      return `(${
        e.value.length == 0
          ? "NULL"
          : e.value.map(() => Eo(t, r.placeholderNumber++)).join(",")
      })`;
...
async function hf(e, t) {
  ...
  if (o !== void 0 || n === "0.0.0" || n === "in-memory") {
    ...
    if (!R.ok)
      throw new Error(
        `Failed to fetch stable Prisma version, unpkg.com status ${R.status} ${
          R.statusText
        }, response body: ${(await R.text()) || "<empty body>"}`
      );
    let S = await R.text();
    ...
    async withRetry(t) {
      for (let r = 0; ; r++) {
        ...
        try {
          ...
          this.logEmitter.emit("warn", {
            message: `Attempt ${r + 1}/${vc} failed for ${t.actionGerund}: ${
              i.message ?? "(unknown)"
            }`,
            timestamp: new Date(),
            target: "",
